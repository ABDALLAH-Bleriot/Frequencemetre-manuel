...
class GeneratorActivity : AppCompatActivity() {
    ...
    private fun setFrequency(value: Int, source: String) {
        val clamped = value.coerceIn(1, 300)
        frequency = clamped
        if (source == "text" && value != clamped) {
            updatingText = true
            etFreq.setText(clamped.toString())
            updatingText = false
        }
        if (running) startBar()
    }
    ...
    private fun stop() {
        ...
        thumb.translationX = (track.width - thumb.width).toFloat().coerceAtLeast(0f)
    }
